/**
 * Enhanced Client creates /workers and /tasks znodes; submits 20 tasks, each named
 * /tasks/task-000000000d where d = 0-19; waits until /workers and /tasks have
...
public class Client2 implements Watcher, Closeable {
     ...
     */
    public Client2( String hostPort ) {
   ...
   /**
	...
	client.createBagOfTasks( ); // create /tasks/task-000000000d (d=0-19)
	client.confirmEmptyBag( );  // delete /workers and /tasks
    }

    private String pid = Long.toHexString( ProcessHandle.current( ).pid( ) );
    private final int nTasksSubmitted = 20; // CHANGED: Now 20 tasks instead of 10
     ...
     * /tasks/task-000000000d where d=0-19. Each task has "submitted" as its
     * data
     */
	...
	for ( int i = 0; i < nTasksSubmitted; i++ ) {
        ...
        System.out.println( "Total of " + nTasksSubmitted + " tasks submitted" );
    }
	...
	for ( int i = 0; i < nTasksSubmitted; i++ ) {
	    String taskPath = String.format("/tasks/task-%010d", i);
	    zk.exists( taskPath, taskWatcher );
	    System.out.println( taskPath + " under watch" );
	}
	...
	// /tasks.
	System.out.println( "all " + nTasksSubmitted + " tasks deleted" );
	zk.delete( "/tasks", 0 );
    ...
    Watcher taskWatcher = new Watcher( ) {
		...
		nTasksCompleted++;
		System.out.println( "Task deleted - Progress: " + nTasksCompleted + "/" + nTasksSubmitted );
            }
	    else {
		try {
