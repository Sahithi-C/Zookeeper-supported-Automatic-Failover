...
public class Worker implements Watcher, Closeable {
	...
	try {
		...
		if ( taskStatus.equals( "submitted" ) ) {
		    ...
		    this.zk.setData("/tasks/" + children.get(i), currTime.toString().getBytes(), 0);

		    // YOUR WORK:call zk.getData( ) and its resuults should be saved in taskStatus
		    taskStatus = new String(zk.getData("/tasks/" + children.get(i), false, null));
		    ...
		    Long pastTime = new Long(taskStatus);
			...
			this.zk.setData("/tasks/" + children.get(i), currTime.toString().getBytes(),
                                taskStat.getVersion());
			System.out.println( taskStatus );
